...
async function sendEvents(events) {
  const payload = { data: events, access_token: accessToken };
...
async function sendLeadEvent(lead) {
  ...
  logger.info('Lead event sent', { event_id: event.event_id, zohoId: lead.zohoId });
...
async function sendQualifiedLeadEvent(lead) {
  ...
  const result = await sendWithRetry([event]);
  logger.info('QualifiedLead event sent', { event_id: event.event_id });
  return { success: true, event_id: event.event_id, result };
}
...
async function sendScheduleEvent(deal) {
  ...
  const result = await sendWithRetry([event]);
  logger.info('Schedule event sent', { event_id: event.event_id });
  return { success: true, event_id: event.event_id, result };
}
...
async function sendDealLostEvent(deal) {
  ...
  const result = await sendWithRetry([event]);
  logger.info('DealLost event sent', { event_id: event.event_id });
  return { success: true, event_id: event.event_id, result };
}
...
async function sendDisqualifiedEvent(lead) {
  ...
  const result = await sendWithRetry([event]);
  logger.info('Disqualified event sent', { event_id: event.event_id });
  return { success: true, event_id: event.event_id, result };
}
...
async function sendBatch(events) {
  ...
  for (let i = 0; i < events.length; i += BATCH_SIZE) {
    results.push(await sendWithRetry(events.slice(i, i + BATCH_SIZE)));
  }
  return results;
}
