...
// Zoho CRM webhook — receives lifecycle events from Zoho Workflows
webhookRouter.post('/zoho', verifyZohoSignature, validateZohoWebhook, async (req, res) => {
  // Respond immediately so Zoho doesn't retry on timeout
  ...
  try {
    await eventQueue.add(
      { event_type, record_id, record_data, received_at: Date.now() },
    );
    logger.info('Event queued', { event_type, record_id });
  } catch (err) {
    logger.error('Failed to enqueue event', { event_type, record_id, error: err.message });
  }
});

// Meta webhook verification (GET) and lead notification (POST)
